...
async def test_dedup_does_not_write_rows_through_the_whole_block(db):
    ...
    await db.commit()
    after_first = await _samples(db, SPEND)

    await ingest_one(db, machine_name="desktop",
                     payload=team_payload(USAGE_WITHDRAWN,
                                          captured_at=now + timedelta(seconds=60)))
    await db.commit()
    assert await _samples(db, SPEND) == after_first
    assert await _samples(db, EXTRA) == after_first


async def test_account_saved_for_the_first_time_mid_block_does_not_show_zero(db):
...
async def test_withdrawal_does_not_erase_series_last_known_value(db):
...
async def test_drop_to_no_measurement_is_not_a_stale_read(db):
...
# --------------------------------------------------------------------------- cascade e2e
async def test_cascade_e2e_after_withdrawal_does_not_show_credits_as_on(db):
...
async def test_extra_usage_does_not_draw_a_second_credits_row(db):
    ...
    assert by[SPEND].primary is True, "the pair's winner must not flip"
    assert by[EXTRA].primary is False
    assert by[EXTRA].duplicate_of == SPEND


async def test_dimmed_row_still_carries_last_measured_percent(db):
    ...
    assert eu.primary is False, "row dimmed"
    assert eu.unavailable_reason == "org_level_disabled_until"
    assert eu.raw_utilization == pytest.approx(92.656), "the number survived the row being dimmed"
...
async def test_withdrawal_and_return_leave_events_with_level_and_details(db):
...
async def test_withdrawal_writes_event_once_not_on_every_measurement(db):
...
async def test_first_measurement_in_a_block_reports_no_transition(db):
...
async def test_reason_from_client_cache_does_not_eclipse_in_band_data(db):
