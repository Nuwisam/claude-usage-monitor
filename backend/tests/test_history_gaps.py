...
def test_no_batches_means_client_silence():
    ...
    ts = m(0, 5, 10, 15, 20)
    gaps = _find_gaps(T0, END, batch_times=ts, sample_times=ts, threshold=GAP_THRESHOLD)
...
def test_batches_exist_but_no_series_samples_is_a_failure_not_idleness():
    ...
    gaps = _find_gaps(T0, END, batch_times=batches, sample_times=samples, threshold=GAP_THRESHOLD)
...
def test_an_empty_range_is_one_silence_for_the_whole_span():
    gaps = _find_gaps(T0, END, batch_times=[], sample_times=[], threshold=GAP_THRESHOLD)
    assert kinds(gaps) == [("client_silent", T0, END)]


def test_dense_work_without_breaks_gives_no_gaps():
    ts = m(*range(0, 481, 5))
    assert _find_gaps(T0, END, batch_times=ts, sample_times=ts, threshold=GAP_THRESHOLD) == []
