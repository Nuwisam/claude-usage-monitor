...
| **Karta albo znacznik nie gasną** na panelu | Zawieszony wpis stanu, bo odmowa i Esc nie generują żadnego zdarzenia hooka. Cztery przypadki, osobno: (1) **sesja już nie żyje** — gaśnie przy najbliższym `UserPromptSubmit`/`Stop` **dowolnej** sesji na tej maszynie, bo jej rekordu nie ma już w `~/.claude/sessions/`; (2) **sesja żyje, ale po odmowie zamilkła** — gaśnie tak samo, tylko dowodem jest `tool_result` w jej transkrypcie; (3) wywołanie, którego rekord `tool_use` nie mieści się w ogonie 32 KB (duży `Write`, ~1,3%) — zostaje TTL 24 h; (4) zabita **ostatnia** sesja na maszynie (nikt nie sprząta rejestru) albo WSL — też TTL. Gdy w logu sondy widzisz `alert_skip`, mechanizm celowo się wstrzymał i powód jest w tej samej linii. Na żądanie: `del %LOCALAPPDATA%\claude-usage-monitor\session-status\*` albo `"session_status": false` w `config.json`, co przy okazji wysyła pusty zbiór |
